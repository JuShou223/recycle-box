    ...
    return ranking.map((user) => (
      <View
        ...
        className={`bg-white rounded-xl p-4 mb-3 transition-all ${
          user.isMe
            ? "border-2 border-green-500 bg-gradient-to-r from-green-50 to-green-100"
            : "shadow-sm"
        }`}
      >
        <View className="flex items-center mb-3">
          <View
            className="text-2xl font-bold w-10 text-center mr-3"
            style={{ color: getRankColor(user.rank) }}
          >
            {getRankIcon(user.rank)}
          </View>
          <Avatar size="medium" src={user.avatar} className="mr-12" />
          <View className="flex-1">
            <View className="flex items-center gap-8 mb-4">
              <Text className="text-14 font-bold text-gray-800">
                {user.nickname}
              </Text>
              {user.isMe && (
    ...
    <View className="min-h-screen bg-gray-50">
      ...
      <View className="mx-20">
        <Tabs value={activeTab} onChange={setActiveTab}>
          <TabPane title="本周排行">
            <View>{renderRankingList(weeklyRanking)}</View>
          </TabPane>

          <TabPane title="本月排行">
            <View>{renderRankingList(monthlyRanking)}</View>
          </TabPane>

          <TabPane title="总排行">
            <View>{renderRankingList(totalRanking)}</View>
          </TabPane>
        </Tabs>
      </View>
      ...
      <View className="m-20 bg-yellow-50 rounded-12 p-16 border border-yellow-200">
        ...
        <View>
          <Text className="text-12 text-yellow-800 block mb-4 leading-relaxed">
            • 积分越高排名越靠前
          </Text>
          <Text className="text-12 text-yellow-800 block mb-4 leading-relaxed">
            • 每日24:00更新排行榜
